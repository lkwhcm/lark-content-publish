...
description: 将发布员提供的单个或多个作品标题、链接、博主、数据员、发布日期、平台、业务板块、项目属性和 KPI 等信息登记到飞书「数据部门 · 内容数据采买与投流台账」的「内容表」；写入后的数据员通知由多维表格自动化处理。用户说“登记作品”“批量上传作品”“录入发布内容”或“把这些作品填进多维表格”时使用；不用于回填每日实际数据或采买明细。
---

# 飞书作品登记

把发布员的自然语言整理为一条可追踪的正式内容记录。调用本 Skill 代表用户要完成校验和写表；写入后由飞书多维表格自动化通知对应数据员，Skill 不直接发送消息。
...
只申请本流程所需的精确权限：读取字段、读取记录、新建记录。不要申请即时消息权限，也不要改成 `--domain base,contact,im`。不要切换或删除用户的 CLI profile。
...
- `production_ready: true`：与 `registration_ready` 同义，仅表示本 Skill 的登记前置条件完整，不代表通知自动化或数据采集器已运行；
...
预览会一次读取表结构和基础数据，校验整批作品，并返回每条最终写入字段、博主匹配和数据员身份。出现以下任一情况时整批停止并一次性向发布员询问：
...
用户的“登记/录入/提交这个作品”请求已经授权创建这一条记录；预览无歧义时直接提交，不再重复索取确认：
...
成功后回报所有记录 ID 和写入的关键字段，并说明“通知由飞书多维表格自动化处理”。写表成功即为 Skill 成功；不要尝试直接私聊数据员，也不要因自动化通知状态未知而把结果判定为失败。
...
- 数据员通知完全交由飞书多维表格自动化；Skill 不申请消息权限、不调用消息接口。
